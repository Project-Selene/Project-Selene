export class StorageFS {
	...
	public async readFile(
		...
		try {
			...
			reader.on('data', str =>
				typeof str === 'string' ? writer.write(this.textEncoder.encode(str)) : writer.write(str),
			);
	...
	public async writeFile(
		...
		try {
			...
			await this.fs.promises.mkdir(this.path.dirname(filePath), {
				recursive: true,
			});
	...
	public async delete(
		...
		try {
			await this.fs.promises.rm(this.path.join(source, path), {
				recursive: true,
			});
			new Blob(['{"success":true}'], { type: 'application/json' }).stream().pipeTo(response);
			return true;
