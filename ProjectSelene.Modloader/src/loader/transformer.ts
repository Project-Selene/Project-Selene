				...
				(ctx => {
					function transform(node: tsType.Node): tsType.Node {
						...
						if (ts.isClassExpression(result)) {
							...
							if (result.parameters.length === 1 && ts.isBlock(result.body)) {
											...
											ts.factory.createBlock(
												[
													...
													ts.factory.createExpressionStatement(
														ts.factory.createCallExpression(ts.factory.createIdentifier('__injectEnum'), undefined, [
...
function hookBlock(block: tsType.Block) {
	...
	for (let j = 0; j < body.length; j++) {
		...
		} else if (ts.isVariableStatement(stmt) && stmt.declarationList.flags & ts.NodeFlags.Let) {
			for (const dec of stmt.declarationList.declarations) {
				if (ts.isIdentifier(dec.name)) {
					...
					body.splice(
						...
						0,
						ts.factory.createExpressionStatement(
							ts.factory.createCallExpression(ts.factory.createIdentifier('__injectLet'), undefined, [
								ts.factory.createStringLiteral(dec.name.text),
								ts.factory.createFunctionExpression(
									undefined,
									undefined,
									...
									[],
									undefined,
									ts.factory.createBlock([ts.factory.createReturnStatement(dec.name)], false),
								),
								ts.factory.createFunctionExpression(
									undefined,
									undefined,
									...
									ts.factory.createBlock(
										[
											ts.factory.createExpressionStatement(
												ts.factory.createAssignment(dec.name, ts.factory.createIdentifier('value')),
											),
										],
										false,
									),
