...
export function ModOptionsEntry(props: { id: string }) {
	...
	return (
		<Accordion expanded={modExpanded} onClick={() => dispatch(toggleModOptionsExpanded(props.id))}>
			<AccordionSummary expandIcon={<ExpandMore />} sx={{ flexDirection: 'row-reverse', gap: 1 }}>
				<Stack direction="row" alignItems="baseline" justifyContent="space-between" width="100%">
					<Typography variant="subtitle1">{modInfo.currentInfo.name}</Typography>
					<Switch
						checked={modEnabled}
						onChange={(_, checked) => dispatch(setModEnabled({ id: props.id, enabled: checked }))}
