...
async function exeuteTestCommand(command: 'loadStoreState', state: RootState): Promise<void>;
async function exeuteTestCommand(
	...
	switch (command) {
		...
		case 'loadStoreState':
			return loadStoreState(args[0] as RootState);
		default:
...
Object.assign(globalThis, {
	sendTestCommand: (args: { command: 'setup', args: [] }) => {
